 ...
 * quando está em dia (< 6h), só chama atenção quando o sync automático parece ter parado. */
function IndicadorSync() {
  ...
  return (
    <span
      title={`Última sincronização do SGE há ${texto} — normalmente roda a cada 3h. Pode ser um problema no workflow automático.`}
...
export default function Layout() {
  ...
  // Retrair a sidebar inteira (só ícones) — diferente de colapsar uma seção: aqui é
  // preferência estável, então persiste entre recarregamentos (ao contrário das seções,
  // que sempre voltam minimizadas).
  const [sidebarColapsada, setSidebarColapsada] = useState(() => {
    try {
      return localStorage.getItem('amorin_sidebar_colapsada') === '1'
    } catch {
      return false
    }
  })
  const alternarSidebar = () => {
    setSidebarColapsada((prev) => {
      const next = !prev
      try {
        localStorage.setItem('amorin_sidebar_colapsada', next ? '1' : '0')
      } catch {
    ...
    >
      ...
      <header className="fixed top-0 left-0 right-0 h-[60px] z-40 bg-[#0a0f14]/80 backdrop-blur-md border-b border-white/[0.06] flex items-center justify-between px-4 lg:px-8">
        ...
        <div className="flex items-center gap-3">
          ...
          <div
            className={`hidden md:flex items-center gap-2 transition-[padding] duration-200 ${
              sidebarColapsada ? 'md:pl-[64px]' : 'md:pl-[248px]'
            }`}
          >
      ...
      {/* Sidebar Desktop Fixa (260px, ou 76px retraída) */}
      <aside
        className={`hidden md:flex flex-col fixed top-0 left-0 bottom-0 bg-[#0a0f14] border-r border-white/[0.06] z-50 transition-[width] duration-200 ${
          sidebarColapsada ? 'w-[76px]' : 'w-[260px]'
        }`}
      >
        {/* Botão de retrair/expandir — meio-afundado na borda direita da sidebar */}
        <button
          type="button"
          onClick={alternarSidebar}
          title={sidebarColapsada ? 'Expandir menu' : 'Retrair menu'}
          aria-label={sidebarColapsada ? 'Expandir menu' : 'Retrair menu'}
          className="absolute -right-3 top-[72px] w-6 h-6 rounded-full bg-[#111820] border border-white/[0.12] flex items-center justify-center text-slate-400 hover:text-orange-400 hover:border-orange-500/40 shadow-md transition-colors z-10"
        >
          <ChevronLeft className={`w-3.5 h-3.5 transition-transform duration-200 ${sidebarColapsada ? 'rotate-180' : ''}`} />
        </button>

        {/* Topo da Sidebar: Logo */}
        <div className={`h-[60px] flex items-center border-b border-white/[0.06] ${sidebarColapsada ? 'justify-center px-2' : 'px-6 gap-3'}`}>
          {configuracoes.logoUrl ? (
            <img
              src={configuracoes.logoUrl}
              alt="Amor In Formaturas"
              className={sidebarColapsada ? 'h-8 max-w-[40px] object-contain' : 'h-9 max-w-[190px] object-contain'}
            />
          ) : sidebarColapsada ? (
        ...
        <nav className={`flex-1 py-4 space-y-1 overflow-y-auto ${sidebarColapsada ? 'px-2' : 'px-3'}`}>
          {visibleSections.map((sec, idx) => {
            // Retraída: sem cabeçalho de seção pra clicar, então mostra tudo direto (só ícones).
            const aberta = sidebarColapsada || secaoEstaAberta(sec)
            return (
              <div key={sec.section ?? `sec-${idx}`} className={idx > 0 ? 'pt-3' : ''}>
                {sec.section && !sidebarColapsada && (
                  <button
                    type="button"
                ...
                {sec.section && sidebarColapsada && idx > 0 && (
                  <div className="border-t border-white/[0.06] mx-1 mb-2" />
                )}
                {aberta &&
                  sec.items.map((item) => {
                    ...
                    return (
                      <Link
                        ...
                        className={`relative flex items-center rounded-lg text-sm font-medium transition-all duration-150 group ${
                          sidebarColapsada ? 'justify-center px-2 py-2.5' : 'gap-3 px-3.5 py-2.5'
                        } ${
                          isActive
                      ...
                      >
                        ...
                        />
                        {!sidebarColapsada && <span className="tracking-tight">{item.label}</span>}
                      </Link>
                    )
                  })}
              </div>
        ...
        <div className="p-3 border-t border-white/[0.06]">
          <div
            className={`flex items-center p-2 rounded-xl bg-[#111820] border border-white/[0.06] group hover:border-white/[0.12] transition-colors ${
              sidebarColapsada ? 'justify-center' : 'justify-between'
            }`}
          >
            <div className={`flex items-center min-w-0 ${sidebarColapsada ? '' : 'gap-2.5'}`}>
              <div
                className="w-9 h-9 rounded-full flex items-center justify-center text-xs font-bold text-white flex-shrink-0 shadow-inner"
                style={{ backgroundColor: currentUser.avatarColor || '#F97316' }}
                title={sidebarColapsada ? `${currentUser.name} (${currentUser.role})` : undefined}
              >
                {initials}
              </div>
              {!sidebarColapsada && (
      ...
      <main
        className={`flex-1 pt-[60px] flex flex-col min-w-0 max-w-full overflow-x-hidden transition-[padding] duration-200 ${
          sidebarColapsada ? 'md:pl-[76px]' : 'md:pl-[260px]'
        }`}
      >
