...
-- parte das vendas e quase todas as contas a pagar, e existem
-- lançamentos manuais sem projeto/turma que vão direto pra contas a
-- receber/pagar sem passar por uma "venda" formal. Por isso o total
-- faturado soma de `pagamentos` (= contas a receber, mesma fonte do
-- Financeiro e do DRE), não de `vendas` — só assim pega 100% dos
-- lançamentos, com ou sem turma, formal ou manual.
drop view if exists public.vw_totais_negocio;
create view public.vw_totais_negocio as
select
  coalesce((select sum(valor) from public.pagamentos where status <> 'cancelado'), 0)::numeric(14,2) as total_faturado,
